...
void initialize(int device) {
    ...
    workspace.matched_sglang = matched_root != nullptr && matched_root[0] != '\0';
    const char * root_env = workspace.matched_sglang ? matched_root :
        std::getenv("ESCHA_GDN_CHUNK_CUBIN_ROOT");
    if (root_env == nullptr || root_env[0] == '\0') {
        throw std::runtime_error("ESCHA_GDN_CHUNK_CUBIN_ROOT is required");
    }
    ...
    if (workspace.matched_sglang) {
        ...
        allocate((void **) &workspace.chunk_indices16, 2*(T/16)*sizeof(int));
        allocate((void **) &workspace.initial_state_indices, sizeof(int));
        int indices64[2*NT], indices16[2*(T/16)];
        for (int i=0;i<NT;i++) { indices64[2*i]=0; indices64[2*i+1]=i; }
        for (int i=0;i<T/16;i++) { indices16[2*i]=0; indices16[2*i+1]=i; }
...
extern "C" int escha_gdn_chunk_prefill(
    ...
    try {
        ...
        if (workspace.matched_sglang) {
            ...
            trace("solve16");
            launch(workspace.solve.function,dim3(T/16,H,1),1,0,as,stream);
