...
$vsRoot = if (Test-Path $vswhereExe) { & $vswhereExe -latest -products * -property installationPath | Select-Object -First 1 } else { $null }
$vcvarsPath = if ($vsRoot) { Join-Path $vsRoot "VC\Auxiliary\Build\vcvarsall.bat" } else { "C:\Program Files (x86)\Microsoft Visual Studio\2022\BuildTools\VC\Auxiliary\Build\vcvarsall.bat" }
