...
static std::atomic<uint64_t> g_kvarn_debug_store_config_logs{0};
...
void ggml_cuda_op_kvarn_store(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    const char * debug_store_env = std::getenv("GGML_KVARN_TEST_DEBUG_STORE_CONFIG");
    if (debug_store_env != nullptr && std::atoi(debug_store_env) != 0 &&
            g_kvarn_debug_store_config_logs.fetch_add(1, std::memory_order_relaxed) < 16) {
        std::fprintf(stderr,
            "KVarN store config: bits=%d value=%d current=%lldx%lldx%lld stage=%lldx%lldx%lld records=%lldx%lldx%lld indices=%lld head_slices=%d n_stream=%d groups_per_stream=%d stage_groups=%d tail_groups=%d eager=%d swa=%d use_workspace=%d use_direct=%d smpbo=%zu\n",
            bits, value ? 1 : 0,
            (long long) current->ne[0], (long long) current->ne[1], (long long) current->ne[2],
            (long long) stage->ne[0], (long long) stage->ne[1], (long long) stage->ne[2],
            (long long) records->ne[0], (long long) records->ne[1], (long long) records->ne[2],
            (long long) indices->ne[0], head_slices, n_stream, groups_per_stream,
            stage_groups, tail_groups, eager_records ? 1 : 0, swa ? 1 : 0,
            use_workspace ? 1 : 0, use_direct ? 1 : 0, smpbo);
    }
