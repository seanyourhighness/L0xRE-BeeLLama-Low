...
#define KVARN_V_AXIS_OTHER(sl, i) __half2float(axes_sh[sl][2 * V_ROWS + (i)])
    ...
    const bool k_from_record = disable_fast_records == 0 && k_tile.fast;
    const bool v_from_record = disable_fast_records == 0 && v_tile.fast;
...
static void ggml_cuda_fattn_kvarn_decode_launch_geometry(
    ...
    ggml_cuda_fattn_kvarn_decode_mma_kernel<D, MAX_GQA, SPLIT_TOKENS, NWARPS, K_BITS, V_BITS, Q_TILE>
        <<<blocks_split, dim3(args.wave_size, NWARPS, 1), 0, args.stream>>>(
            ...
            args.n_splits, args.disable_fast_records);
}
