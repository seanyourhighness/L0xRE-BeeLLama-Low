...
static bool ggml_backend_cpu_device_supports_op(ggml_backend_dev_t dev, const struct ggml_tensor * op) {
    ...
    switch (op->op) {
        ...
        case GGML_OP_LOWGPU_MUL_MAT:
            // Round 1: the packed 3-bit LM head executes only on CUDA. A full
            // V x n_embd F16 expansion is forbidden by the Escha DoD, so the
            // CPU backend must reject placement before execution.
            return false;
