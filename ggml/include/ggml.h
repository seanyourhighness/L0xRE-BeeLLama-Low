...
extern "C" {
    ...
    */
    ...
    // Use compact grouped Q/K heads for the fused Escha decode kernel.  The
    // CUDA implementation performs the head expansion and per-head L2 norm.
    GGML_API void ggml_gated_delta_net_set_grouped_qk_norm(
            struct ggml_tensor * tensor, float eps);

    // Fuse the Escha decode alpha/beta preparation into the CUDA GDN kernel.
    // The setter attaches the raw dt and A_log vectors as graph dependencies.
    GGML_API void ggml_gated_delta_net_set_decode_prep(
            struct ggml_tensor * tensor,
            struct ggml_tensor * dt,
            struct ggml_tensor * a_log);
    ...
    // Dense Escha graph-only auxiliary binding.  The auxiliary tensor is not
    // part of the ordinary projection semantics; it is consumed only by an
    // explicitly matched backend fusion.
    GGML_API struct ggml_tensor * ggml_escha_mul_mat_with_aux(
            struct ggml_context * ctx,
            struct ggml_tensor  * code,
            struct ggml_tensor  * rin,
            struct ggml_tensor  * rout,
