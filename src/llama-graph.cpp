...
const llm_escha_tables & tab,
  const llm_escha_exps & w,
    ...
    // The external raw-decode bridge accepts only all-output decode/verify graphs.
    // A short prefill can have the same row count while requesting only the final logit.
    out->op_params[0] = (int32_t) (n_outputs == n_tokens);
...
ggml_tensor * llm_graph_context::build_escha_mm_aux(
const llm_escha_tables & tab,
  const llm_escha_exps & w,
          ggml_tensor * cur,
          ggml_tensor * aux) const {
    const int64_t K = w.code->ne[0]/16;
    ggml_tensor * dep = K == 2 ? tab.dep_k2 : tab.dep_k3;
    GGML_ASSERT(dep != nullptr && aux != nullptr);

    ggml_tensor * out = ggml_escha_mul_mat_with_aux(ctx0, w.code, w.rin, w.rout,
                                                     tab.lut, dep, cur, aux);
    out->op_params[0] = (int32_t) (n_outputs == n_tokens);
...
ggml_tensor * llm_graph_context::build_inp_embd(
    ...
    // token embeddings path (ubatch.token != nullptr)
    {
        ...
        const bool has_dense = tok_embd != nullptr;
        const bool any_packed = lg_code != nullptr || lg_scale != nullptr || lg_zp != nullptr;
        const bool complete_packed = lg_code != nullptr && lg_scale != nullptr && lg_zp != nullptr;
        if (has_dense == complete_packed || any_packed != complete_packed) {
            throw std::runtime_error("input embedding requires either one dense tensor or one complete LowGPU tensor set");
        }
        ...
        // Fail closed: a LowGPU-packed embedding has no dense weight to add a low-rank
        // update to, so an active LoRA over it is unsupported in Round 1 (no silent skip).
        if (tok_embd == nullptr) {
            if (!loras->empty()) {
