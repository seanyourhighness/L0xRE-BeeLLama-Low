...
static llama_model * llama_model_mapping(llm_arch arch, const llama_model_params & params) {
    switch (arch) {
        ...
        case LLM_ARCH_ESCHA:
            return new llama_model_escha(params);
...
bool llama_model_base::load_tensors(llama_model_loader & ml) {
    ...
    for (auto & [ctx_key, ctx_ptr] : ml.ctx_map) {
        ...
        bool has_transformed_tensor = false;
        for (ggml_tensor * tensor = ggml_get_first_tensor(ctx); tensor != nullptr; tensor = ggml_get_next_tensor(ctx, tensor)) {
            if (ml.find_transform(ggml_get_name(tensor)) != nullptr) {
                has_transformed_tensor = true;
                break;
            }
        }
        ...
        default:
            {
                ...
                const bool mtp_on_hybrid_qwen =
                    ...
                    (arch == LLM_ARCH_QWEN3NEXT || arch == LLM_ARCH_QWEN35 || arch == LLM_ARCH_QWEN35MOE ||
                     arch == LLM_ARCH_QWEN4EXP || arch == LLM_ARCH_BAILINGMOE3 || arch == LLM_ARCH_ESCHA);
                ...
                if (llm_arch_is_recurrent(arch)) {
                    ...
                    } else if (arch == LLM_ARCH_QWEN3NEXT || arch == LLM_ARCH_QWEN35 || arch == LLM_ARCH_QWEN35MOE || arch == LLM_ARCH_QWEN4EXP || arch == LLM_ARCH_MINIMAX_01 || arch == LLM_ARCH_ESCHA) {
                        filter_attn = [&](uint32_t il) {
